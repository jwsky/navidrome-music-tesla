...
## GET /correct?q=<关键词>（可选）

让后端在搜索前把关键词顺一遍，主要用于语音输入产生的同音字。放在服务端的好处是
模型的 key 不必出现在浏览器里，而且后端可以对重复查询做缓存。

```jsonc
{
  "original":  "告白汽球",
  "corrected": "告白气球",
  "changed":   true          // false = 不用改
}
```

`changed` 为 true 时，前端会**同时**搜索原词和纠正词，两边结果都保留。
没有这个端点时，前端退回设置里配置的 OpenAI 兼容接口（如果填了的话）。

## 预留

还有两个端点在计划中、尚未实现，列在这里是为了让后端避开这两个名字：
`/asr` 接收录音返回文字，`/inbox` 供外部触发（比如苹果快捷指令）投递一条待搜索的关键词。
详见 README 里的开发计划。
