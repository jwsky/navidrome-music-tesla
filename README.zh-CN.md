...
手机没有并排两栏的宽度，所以歌词占满整屏，曲库变成底部的浮层，收起时只露出标题那一条。

| | |
| --- | --- |
| ![浮层](docs/mobile-sheet.png) | ![播放](docs/mobile-player.png) |
...
上面两张图跑在公开演示库上。当时那台服务器的封面接口没有响应，所以显示的是内置占位图——
封面是先把图取回来、确认加载成功才替换的，服务器慢或者挂掉只会停在占位图，不会留下一片空白块。
